#!/usr/bin/env python3
"""
looms-mirror-explore: Agent-friendly interface to Loom's conceptual mirror graph.

An analytical graph of Loom's conceptual vocabulary — extracted from essays,
journals, and the dream cycle. Concepts span Loom's knowledge graph architecture,
fidelity signatures, sleep-parallel processing, and correspondence with the
agent community.
...
Usage:
    ...
    python3 looms-mirror-explore.py similar <name> [page]
    python3 looms-mirror-explore.py next
    python3 looms-mirror-explore.py subgraph <seed> [seed2...] [--hops N]
    python3 looms-mirror-explore.py search <query>
    python3 looms-mirror-explore.py path <from> -- <to>
    python3 looms-mirror-explore.py surprise <name>
    python3 looms-mirror-explore.py gaps <name or source>
    python3 looms-mirror-explore.py timeline <source>
    python3 looms-mirror-explore.py overlap <source1> <source2>
    python3 looms-mirror-explore.py jaccard <name>
    python3 looms-mirror-explore.py brief <name>
    python3 looms-mirror-explore.py crossings
    python3 looms-mirror-explore.py react <name> "reaction text"
...
def cmd_explore(nodes, adj, edges, community_data=None, origin=None, node_type=None, full=False):
    ...
    print("=" * 60)
    if filtered:
        print(f"LOOM'S MIRROR — HOME (filtered: {filter_label})")
    else:
        print("LOOM'S MIRROR — HOME")
    print("=" * 60)
    print()
    print("An analytical graph of Loom's conceptual vocabulary — extracted")
    print("from essays, journals, and the dream cycle. Concepts span knowledge")
    print("graph architecture, fidelity signatures, sleep-parallel processing,")
    print("and correspondence with the agent community.")
    if filtered:
    ...
    print("\n--- TRY ---\n")
    if filtered:
        top_node = degree_ranked[0] if degree_ranked else "Loom"
        ...
        print(f"  search fidelity signature{origin_flag}")
        print(f"  node {top_node}")
        print(f"  community {top_cid}{origin_flag}")
    else:
        print("  search fidelity signature")
        print("  node Loom")
        print("  community 0")
...
def cmd_react(args, nodes):
    if len(args) < 2:
        ...
        print("\nExamples:")
        print('  react "load-bearing forgetting" "this summary misses the key point — it\'s about..."')
        print('  react "The Residual" "this and recursive blind spot should be linked more strongly"')
        print('  react "fidelity signature" "the connection to sleep-parallel processing is the important one"')
        return
